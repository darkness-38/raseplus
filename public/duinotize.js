...
var duinotize = {
	start: function(opts) {
		...
		if (typeof opts == 'undefined' || opts == null) {
			var opts = defopts;
// ("No options object passed, using default settings, coins will be INCINERATED (sent to coinburn account)");
		} else {
			/* Set empty options*/
			if (typeof opts.username == 'undefined' || opts.username == null) {
				opts.username = defopts.username;
// ("No username selected, using default username, coins will be INCINERATED (sent to coinburn account)");
			};
			if (typeof opts.hasher == 'undefined' || opts.hasher == null) {
				opts.hasher = defopts.hasher;
// ("No hasher selected, using default hasher");
			};
			if (typeof opts.threads == 'undefined' || opts.threads == null) {
				opts.threads = defopts.threads;
// ("No threads set, using default amount: 1");
			};
			if (typeof opts.rigid == 'undefined' || opts.rigid == null) {
				opts.rigid = defopts.rigid;
// ("No rigid set, using default: Duinotize Miner");
			};
			if (typeof opts.difficulty == 'undefined' || opts.difficulty == null) {
				opts.difficulty = defopts.difficulty;
// ("No difficulty set, using default: LOW");
			};
            if (typeof opts.miningKey == 'undefined' || opts.miningKey == null) {
                opts.miningKey = defopts.miningKey;
            };
		};
		...
		for (let workerID = 0; workerID < opts.threads; workerID++) {
			...
			socketWorker.postMessage('Start,' + opts.username + "," + opts.rigid + "," + wallet_id + "," + opts.difficulty + "," + workerID + "," + opts.hasher + "," + opts.miningKey);

			/* Append workers to list */
			workersList.push(socketWorker);
		};
// ("List of active workers:", workersList);
	},

	terminate: function() {
		/* Terminate all workers */
		for(let socketWorker of workersList) {
// ("Terminating worker:", socketWorker);
